#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#define MAXSIZE 5
typedef struct{
  char name[19];
  int stk[MAXSIZE];
  int top;
}mhs;
typedef struct stack STACK;
mhs student[100];

char riceType[98];

int sellRice();
void enter(int x);
void getMenu();
void stockRice();

void initCake();
void insertRice();
int main (){
  //  initCake();
  student[0].top = -1;
  getMenu();	
  return 0;
}

void insertRice(){
  int num;
  if (student[0].top == (MAXSIZE - 1))
  {
    printf ("Stack is Full\n");
    return;
  }
  else
  {
    
    char nama [19];
    int top;
    top = student[0].top =  student[0].top + 1;
    //gets(student[0].name);
    printf("Input Rice type [ long / medium / short ]");
    scanf("%s", student[top].name);
    while (strcmp (student[top].name,"long")  != 0  && strcmp (student[top].name,"medium")  != 0  && strcmp (student[top].name,"short")  != 0  ){
      printf("Input Rice type [ long / medium / short ]");
      scanf("%s",student[top].name);
    }
    printf ("Input weight of the rice sack [10.. 100 kg<s> ] :\n");
    scanf ("%d", &num);
    while(num>= 100 || num<=10){
      printf ("Input weight of the rice sack [10.. 100 kg<s> ] :\n");
      scanf ("%d", &num);
    }
    student[0].stk[student[0].top] = num;
  }
  system("pause");
  getMenu();	
  
}
void initCake(){
  // strcpy(student[0].name ,"Kaka")	;
  // student[0].stk[0]=12;
}
void enter(int x){
  for(int i=0; i<x;i++){
    printf("\n");
  }
}
void getMenu(){
  
  printf("Blue Rice Stock \n");	
  
  char * menu[] ={"Option :", "1. Stock Rice Sack", "2. Sell Rice","3. Exit"};
  stockRice();
  for(int i =0; i< 4; i++ ){
    printf("%s \n",menu[i]);
  }
  int pilihan;
  scanf("%d", &pilihan);
  if (pilihan == 1 ) insertRice();
  if (pilihan == 2) sellRice();
  
}
void stockRice(){
  int i;
  if ( student[0].top == -1)
  {
    printf ("Stack is empty\n");
    return;
  }
  else
  {
    printf ("\n The status of the stack is \n");
    if ( student[0].top == student[0].top){
      printf ("%s %d ",  student[student[0].top].name, student[0].stk[student[0].top] );
      printf("top\n");
    }
    for (i =  student[0].top; i >= 1; i--)
    {
      printf ("%s %d \n",  student[i-1].name, student[0].stk[i-1] );
      
    }
  }
  printf ("\n");
}
int sellRice(){
  int num;
  if ( student[0].top == - 1)
  {
    printf ("Stack is Empty\n");
    return ( student[0].top);
  }
  else
  {
    num =  student[0].stk[ student[0].top];
    printf ("poped element is = %dn",  student[0].stk[ student[0].top]);
    student[0].top =  student[0].top - 1;
  }
  system("pause");
  getMenu();	
  
  return(num);
  
}
